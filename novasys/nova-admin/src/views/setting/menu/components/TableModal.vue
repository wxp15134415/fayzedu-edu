...
async function submitModal() {
  ...
  const submitData = {
    title: formModel.value.title || formModel.value.name,
    name: formModel.value.name,
    parentId: formModel.value.pid || 0,
    menuType: formModel.value.menuType === 'dir' ? 'directory' : 'page',
    sort: formModel.value.order || 0,
    path: formModel.value.path || '',
    component: formModel.value.componentPath || '',
  ...
  try {
    ...
    if (modalType.value === 'add') {
      ...
      await updateMenu(formModel.value.id, submitData)
