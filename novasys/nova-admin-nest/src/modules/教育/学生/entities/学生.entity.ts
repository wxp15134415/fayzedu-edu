...
export class Student {
  ...
  @Column({ length: 30, unique: true, comment: '学号' })
  xuehao: string
  ...
  @Column({ length: 100, nullable: true, comment: '籍贯/国籍' })
  jiguandanwei: string

  @Column({ length: 200, nullable: true, comment: '家庭地址' })
  jiating_dizhi: string

  @Column({ type: 'integer', nullable: true, comment: '关联家长ID' })
  jiazhang_id: number | null

  @Column({ type: 'integer', nullable: true, comment: '关联班级ID' })
  banji_id: number | null

  @Column({ type: 'date', nullable: true, comment: '入学日期' })
  ruxue_riqi: Date

  @Column({ type: 'date', nullable: true, comment: '毕业日期' })
  biye_riqi: Date

  @Column({ length: 30, nullable: true, comment: '学籍账号' })
  xuejizhanghao: string

  @Column({ length: 100, nullable: true, comment: '健康状况' })
  jiankangzhuangkuang: string

  @Column({ length: 500, default: '', comment: '备注' })
  beizhu: string

  @Column({ type: 'smallint', default: 0, comment: '状态（0在读 1休学 2毕业 3退学）' })
  zhuangtai: number
