...
async function testDeptPage() {
  ...
  try {
    ...
    console.log('📊 侧边栏是否包含设置:', menuText.includes('系统设置'));
    console.log('📊 侧边栏是否包含部门:', menuText.includes('部门'));
    
    console.log('\n🔄 尝试展开设置菜单并点击部门设置...');
    
    const settingMenuItems = await page.$$('.n-menu-item');
    console.log(`📊 找到 ${settingMenuItems.length} 个菜单项`);
    
    for (const item of settingMenuItems) {
      const text = await item.textContent();
      if (text && text.includes('系统设置')) {
        console.log('📍 找到系统设置，尝试展开...');
        await item.click();
        await page.waitForTimeout(2000);
        break;
      }
    ...
    await page.screenshot({ path: `${screenshotsDir}/dept-menu-expanded-${timestamp}.png`, fullPage: true });
    
    const afterExpandText = await page.textContent('body');
    console.log('📊 展开后是否包含部门设置:', afterExpandText.includes('部门'));
    
    const deptItems = await page.$$('.n-menu-item');
    for (const item of deptItems) {
      const text = await item.textContent();
      if (text && (text.includes('部门设置') || text.includes('部门'))) {
        console.log(`📍 找到部门菜单项: "${text.trim()}"`);
        await item.click();
    ...
    console.log(`  - 菜单展开: ${screenshotsDir}/dept-menu-expanded-${timestamp}.png`);
