...
async function testMenuPage() {
  ...
  try {
    ...
    // 截图登录后页面
    await page.screenshot({ path: `${screenshotsDir}/menu-after-login-${timestamp}.png`, fullPage: true });
    ...
    // 截图菜单管理页面
    await page.screenshot({ path: `${screenshotsDir}/menumanagement-${timestamp}.png`, fullPage: true });
    console.log('菜单管理页面截图已保存');
