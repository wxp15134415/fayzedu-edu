...
async function closeModal(page) {
  try {
    const closeBtn = await page.$('.n-modal-close, .n-modal-close-button, [class*="close"], button[class*="close"]');
    if (closeBtn) {
...
async function waitForModalClose(page, timeout = 5000) {
  try {
    await page.waitForFunction(() => {
      ...
      return !modal || modal.getAttribute('aria-hidden') === 'true' || !modal.isConnected;
    }, { timeout });
    await page.waitForTimeout(500);
  } catch (e) {
    console.log('等待模态框关闭超时');
  }
}
