...
async function closeModalAggressive(page) {
  try {
    // Method 1: Click close button if exists
    const closeBtn = await page.$('.n-modal-close, .n-modal-close-button, [class*="modal-close"], button[class*="close"]');
    if (closeBtn) {
    ...
    // Method 3: Click outside modal (on mask)
    const mask = await page.$('.n-modal-mask');
    if (mask) {
      await mask.click({ position: { x: 10, y: 10 }, force: true });
      await page.waitForTimeout(500);
    }
  } catch (e) {
    console.log('关闭模态框尝试失败');
  }
}
...
async function waitForModalClose(page, timeout = 5000) {
  ...
  while (Date.now() - startTime < timeout) {
    ...
    const modalExists = await page.evaluate(() => {
      ...
      // Check if it's truly gone (not just hidden)
      const style = window.getComputedStyle(container);
      const display = style.display;
      const visibility = style.visibility;
      const opacity = style.opacity;
      
      // If display:none or visibility:hidden or opacity:0, consider it closed
    ...
    // Try to close it aggressively
    await closeModalAggressive(page);
    await page.waitForTimeout(500);
  }
  
  console.log('等待模态框关闭超时，尝试强制刷新页面...');
...
async function findAndClickButton(page, textPatterns, force = false) {
  const buttons = await page.$$('button');
  for (const btn of buttons) {
    const text = await btn.textContent();
    if (text) {
...
async function fillFormInputs(page, values) {
  try {
    ...
    for (const input of inputs) {
      ...
      if (isVisible) {
        ...
        if (!placeholder.includes('请选择') && !placeholder.includes('日期') && placeholder !== '') {
...
async function testUserCRUD(page) {
  ...
  try {
    ...
    if (foundAdd) {
      ...
      await fillFormInputs(page, [`testuser${timestamp}`, `测试用户${timestamp}`, '123456']);
      ...
      console.log('✓ 添加用户操作已执行');
      testResults.user.add = { success: true, message: '添加用户操作已执行' };
    } else {
      console.log('⚠ 未找到新增按钮');
    ...
    if (foundEdit) {
      ...
      await fillFormInputs(page, [`修改后的昵称${Date.now()}`]);
    ...
    if (foundDelete) {
      ...
      console.log('✓ 删除用户操作已执行');
      testResults.user.delete = { success: true, message: '删除操作已执行' };
    } else {
      console.log('⚠ 未找到删除按钮');
...
async function testRoleCRUD(page) {
  ...
  try {
    ...
    if (foundAdd) {
      ...
      const timestamp = Date.now();
      await fillFormInputs(page, [`测试角色${timestamp}`, `test_role_${timestamp}`]);
      ...
      console.log('✓ 添加角色操作已执行');
      testResults.role.add = { success: true, message: '添加角色操作已执行' };
    } else {
      console.log('⚠ 未找到新增按钮');
    ...
    if (foundEdit) {
      ...
      await fillFormInputs(page, [`修改后的角色${Date.now()}`]);
    ...
    if (foundDelete) {
      ...
      console.log('✓ 删除角色操作已执行');
      testResults.role.delete = { success: true, message: '删除操作已执行' };
    } else {
      console.log('⚠ 未找到删除按钮');
...
async function testMenuCRUD(page) {
  ...
  try {
    ...
    if (foundAdd) {
      ...
      const timestamp = Date.now();
      await fillFormInputs(page, [`测试菜单${timestamp}`, `/test/${timestamp}`]);
      ...
      console.log('✓ 添加菜单操作已执行');
      testResults.menu.add = { success: true, message: '添加菜单操作已执行' };
    } else {
      console.log('⚠ 未找到新增按钮');
    ...
    if (foundEdit) {
      ...
      await fillFormInputs(page, [`修改后的菜单${Date.now()}`]);
    ...
    if (foundDelete) {
      ...
      console.log('✓ 删除菜单操作已执行');
      testResults.menu.delete = { success: true, message: '删除操作已执行' };
    } else {
      console.log('⚠ 未找到删除按钮');
...
async function main() {
  console.log('开始 CRUD 测试 (v4 - 强制关闭模态框版)...\n');
