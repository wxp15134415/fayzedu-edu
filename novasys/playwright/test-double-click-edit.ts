...
async function testDoubleClickEdit() {
  ...
  try {
    ...
    if (contactCellCount > 0) {
      ...
      const editInput = await page.$('td.editable input, td input[type="text"], td [contenteditable="true"], .ant-input, .el-input__inner');
      ...
      if (isEditMode) {
        ...
        const inputEl = await page.$('td.editable input, td input[type="text"], .ant-input, .el-input__inner');
        if (inputEl) {
          await inputEl.click();
          await inputEl.fill('');
          await inputEl.fill('13800138000');
      ...
      if (anyInput) {
        ...
        const editInputAfter = await page.$('input:focus, input.ant-input, .el-input__inner');
        if (editInputAfter) {
