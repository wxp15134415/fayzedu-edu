...
async function testDeptPage() {
  ...
  try {
    ...
    if (!pageText.includes('部门管理')) {
      console.log('🔄 尝试点击侧边栏的"设置"菜单...');
      
      const settingLink = await page.$('a[href*="setting"], .n-menu-item a[href*="setting"]');
      if (settingLink) {
        console.log('📍 找到设置链接，点击...');
      ...
      for (const link of allLinks) {
        ...
        if (href && href.includes('dept')) {
          console.log(`📍 找到部门链接: ${href} - ${text}`);
          await link.click();
          await page.waitForTimeout(3000);
          break;
        }
